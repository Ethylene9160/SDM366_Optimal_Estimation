...
def save_reward_list(reward_list_a, reward_list_b, current_time):
    ...
    plt.subplot(2, 1, 1)
    plt.plot(reward_list_a)
    plt.title('Reward by PPO agent a')
    plt.xlabel('Time step')
    plt.ylabel('Reward')

    plt.subplot(2, 1, 2)
    plt.plot(reward_list_b)
...
if __name__ == "__main__":
    ...
    read_model_b_path = "models_v4/b/temp_1718003498_epoch_5098.zip"
    read_model_a_path = "models_v4/a/temp_1718003498_epoch_5098.zip"

    os.makedirs(f"outputs_v4/{current_time}/a", exist_ok=True)
    os.makedirs(f"outputs_v4/{current_time}/b", exist_ok=True)

    env_a = tools.CustomEnv(model, data, calculate_reward_a)
    env_b = tools.CustomEnv(model, data, calculate_reward_b)
