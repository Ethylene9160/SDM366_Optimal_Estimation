...
def calculate_reward(state_list):
    ...
    if abs(theta) >= 0.25 and (x_dot * theta) > 0:
        r -= 0.01 * x_dot ** 2
        if abs(x) >= 0.5:
            r -= 1
    return r
...
if __name__ == "__main__":
    ...
    try:
        ...
        while episode < total_num_episodes:
            ...
            reward_list.append(calculate_reward(tools.get_obs_lifer(data)))
        ...
        agent.save_model(f"outputs/{current_time}/model_{int(time.time())}_epoch_{episode}")
