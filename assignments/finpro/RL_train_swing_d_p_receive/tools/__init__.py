...
class CustomEnv(gym.Env):
    ...
    def step(self, action):
        ...
        theta = np.arctan2(state[8], state[9])
...
def random_state(data, seed=2333):
    ...
    init_theta1 = np.random.uniform(-0.5, 0.5)
    init_theta2 = np.random.uniform(-0.5, 0.5)
    init_v = np.random.uniform(-2.0, 2.0)
    init_omega1 = np.random.uniform(-2.0, 2.0)
    init_omega2 = np.random.uniform(-2.0, 2.0)
    data.qpos[0] = init_x
...
def get_obs_lifer(data):
    ...
    # state = [x, theta 1, theta 2, v, omega 1, omega 2, sin(theta 1), cos(theta 1), sin(theta 2), cos(theta 2)]
    return state
