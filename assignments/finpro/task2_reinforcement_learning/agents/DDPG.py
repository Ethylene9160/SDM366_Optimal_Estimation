...
class DDPGAgent:
    def __init__(self, no_of_states, no_of_actions, a_bound:float=20.0, l_bound:float = 0.0, lr_a=0.001, lr_c=0.002, gamma=0.9, alpha=0.01, device='cpu'):
        self.no_of_actions = no_of_actions
        self.no_of_states = no_of_states
        self.a_bound = abs(a_bound)
        self.l_bound = abs(l_bound)
    ...
    def choose_action(self, state):
        ...
        action = np.where((action > -self.l_bound) & (action < 0), -self.l_bound, action)
        action = np.where((action < self.l_bound) & (action > 0), self.l_bound, action)
        return action
