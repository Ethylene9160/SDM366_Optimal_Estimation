...
class DDPGAgent:
    def __init__(self, no_of_states, no_of_actions, a_bound = 20.0, lr_a=0.001, lr_c=0.002, gamma=0.9, alpha=0.01):
        ...
        self.actor_eval = self.build_actor_network()
        self.actor_target = self.build_actor_network()
        self.critic_eval = self.build_critic_network()
    ...
    def build_actor_network(self):
        return nn.Sequential(
            nn.Linear(self.no_of_states, 128),  # 输入层到第一个隐藏层
            nn.ReLU(),
            nn.Linear(128, 64),  # 第一个隐藏层到第二个隐藏层
            nn.ReLU(),
            nn.Linear(64, self.no_of_actions),  # 第二个隐藏层到输出层
            nn.Tanh()
        )

    def build_critic_network(self):
        return nn.Sequential(
            nn.Linear(self.no_of_states + self.no_of_actions, 128),  # 输入层到第一个隐藏层
            nn.ReLU(),
            nn.Linear(128, 64),  # 第一个隐藏层到第二个隐藏层
            nn.ReLU(),
            nn.Linear(64, 1)  # 第二个隐藏层到输出层
        )

    def sample_action(self, state):
        return self.choose_action(state), 0
    def choose_action(self, state):
        state = torch.FloatTensor(state).unsqueeze(0)  # 确保输入的维度是 (1, no_of_states)
        action = self.actor_eval(state).detach().numpy()[0]  # 通过网络前向传播获得动作
        action = np.clip(np.random.normal(action, self.noise_variance), -self.a_bound, self.a_bound)  # 添加噪声并裁剪
        # 保证action的每个元素，如果处于(-2,2)区间，则改为-2.0或2.0
    ...
    def learn(self):
        ...
        batch_states = torch.FloatTensor(batch_transition[:, :self.no_of_states])
        batch_actions = torch.FloatTensor(batch_transition[:, self.no_of_states: self.no_of_states + self.no_of_actions])
        batch_rewards = torch.FloatTensor(batch_transition[:, -self.no_of_states - 1: -self.no_of_states])
        batch_next_states = torch.FloatTensor(batch_transition[:, -self.no_of_states:])
